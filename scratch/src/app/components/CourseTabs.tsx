...
const reviews = [
  {
    ...
    rating: 5,
    date: "2 hafta önce",
    text: "Gerçekten premium bir eğitim. İktisat anlatımı sayesinde netlerim 10'dan 25'e çıktı.",
  },
  {
    id: 2,
    name: "Zeynep D.",
    rating: 5,
    date: "1 ay önce",
    text: "Hukuk derslerinden korkuyordum. Anlatım oturttu, sistem akıyor. Çok memnunum.",
  },
  {
    id: 3,
    name: "Mehmet K.",
    rating: 4,
    date: "1 ay önce",
    text: "Genel olarak çok iyi. Notlar biraz daha detaylı olsa mükemmel olurdu.",
  },
];
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <div className="mt-1 flex items-center gap-3">
              ...
              <Stars rating={avg} />
              <div className="text-sm text-dark/60">
                ({reviews.length} örnek yorum)
              </div>
            </div>
            <div className="mt-2 text-sm text-dark/60">
              “Sınav odaklı anlatım” ve “tekrar sistemi” en çok övülen iki nokta.
            </div>
          </div>

          <a
            href="/iletisim"
            className="inline-flex items-center justify-center rounded-2xl px-5 py-3 text-sm font-extrabold text-white btn-4t-secondary"
          >
            Danışmanla Konuş
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {featured.map((review) => (
          <div
            key={review.id}
            className="glass-card p-6 shadow-[0_16px_50px_rgba(11,60,138,0.10)]"
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-dark/50">{review.date}</div>
              </div>
              <Stars rating={review.rating} />
            </div>

            <p className="mt-4 text-base text-dark/70 leading-relaxed italic">
...
export default function CourseTabs({ course }: { course: CourseProps }) {
  const [activeTab, setActiveTab] = useState<
    "genel" | "mufredat" | "egitmenler" | "yorumlar" | "sss"
  >("genel");
  ...
  const tabs = [
    ...
    { id: "yorumlar", name: "Yorumlar", icon: ChatBubbleLeftRightIcon },
