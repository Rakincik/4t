...
export default function Footer() {
  ...
  const footerLinks = {
    ...
    kurumsal: [
      ...
      { name: "Başarılarımız", href: "/basarilarimiz" },
    ...
    >
      ...
      <div className="relative z-10 container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-start gap-3 text-sm text-gray-400 group">
                ...
                <span className="group-hover:text-gray-300 transition-colors whitespace-pre-line">
                  {config?.address || "İlkbahar Mah. 593 Sk. No:2 \n Çankaya / ANKARA"}
                </span>
              </div>
              <a href={`mailto:${config?.email || 'destek@4takademi.com'}`} className="flex items-center gap-3 text-sm text-gray-400 group">
                <EnvelopeIcon className="h-5 w-5 text-[#DC2626] group-hover:scale-110 transition-transform" />
                <span className="group-hover:text-white transition-colors">{config?.email || "destek@4takademi.com"}</span>
              </a>
              <div className="flex items-center gap-3 text-sm text-gray-400 group">
                ...
                <span className="group-hover:text-white transition-colors">{config?.phone || "(0312) 433 40 44"}</span>
              </div>
            </div>
          </div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                  <a
                    ...
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20b857] text-white px-6 py-4 rounded-xl font-bold transition-all hover:scale-105 shadow-lg shadow-green-900/20 whitespace-nowrap"
                  >
                  ...
                  <a
                    href="/flix"
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white px-6 py-4 rounded-xl font-bold transition-all border border-white/5 whitespace-nowrap"
                  >
                    Paketleri İncele
                  </a>
                </div>
        ...
        <div className="mt-20 pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-gray-500 text-sm text-center md:text-left whitespace-pre-line">
            {config?.footerText || `© ${new Date().getFullYear()} 4T Akademi Yayıncılık. Tüm hakları saklıdır.`}
          </div>
