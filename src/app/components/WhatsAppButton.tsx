...
export default function WhatsAppButton() {
  ...
  React.useEffect(() => {
    fetch("/api/settings/global")
      .then(res => res.json())
      .then(data => {
        if (data && data.config?.whatsapp) {
          setPhone(data.config.whatsapp);
        }
      })
      .catch(console.error);
  }, []);
    ...
    <div className="fixed bottom-20 sm:bottom-6 right-6 z-[9999] flex items-end gap-3">
      ...
      {showTooltip && (
        <div className="bg-white shadow-xl rounded-2xl p-4 max-w-[220px] animate-[fadeIn_0.2s_ease-out] border border-gray-100">
          <p className="text-sm font-bold text-gray-800 mb-1">Yardıma mı ihtiyacınız var?</p>
          <p className="text-xs text-gray-500 leading-relaxed">WhatsApp üzerinden bize anında ulaşın.</p>
          <button
      ...
      <a
        ...
        className="group relative w-12 h-12 sm:w-[60px] sm:h-[60px] bg-[#25D366] hover:bg-[#1ebe57] rounded-full flex items-center justify-center shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all hover:scale-110 active:scale-95"
