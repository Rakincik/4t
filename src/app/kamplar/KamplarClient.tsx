...
export default function KamplarClient({ initialCourses, activeCategories = [] }: { initialCourses: any[], activeCategories?: { name: string; slug: string }[] }) {
  ...
  // Gelişmiş Filtreleme (Dinamik Kategoriler)
  const activeCategoriesList = useMemo(() => {
    const list = ["Tümü"];
    activeCategories.forEach(cat => list.push(cat.name));
    return list;
  }, [activeCategories]);
  ...
  const totalPages = Math.ceil(filteredKamplar.length / itemsPerPage);
  const paginatedKamplar = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return filteredKamplar.slice(startIndex, startIndex + itemsPerPage);
  }, [filteredKamplar, currentPage, itemsPerPage]);
          ...
          {filteredKamplar.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {paginatedKamplar.map((course, idx) => (
              ...
              <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                <div className="text-sm text-gray-500 font-medium">
                  Toplam <strong className="text-gray-900">{filteredKamplar.length}</strong> kamp listeleniyor.
                </div>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-bold rounded-lg bg-orange-500 text-white hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"
                      >
                        Sonraki
                      </button>
                    </div>
